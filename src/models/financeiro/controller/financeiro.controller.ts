...
export class FinanceiroController {
  ...
  @ApiResponse({
    ...
    type: ResponseMovimentacaoFinanceiraDTO,
  })
  @ApiResponse({
    status: 400,
    description: 'Erro ao registrar saída financeira',
  })
  ...
  async gerarRelatorioFinanceiro(
    ...
    if (
      ...
      throw new BadRequestException(
        'As datas de início e fim são obrigatórias e devem estar no formato YYYY-MM-DD.',
      );
    }
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de movimentações financeiras retornada com sucesso',
    type: [ResponseMovimentacaoFinanceiraDTO],
  })
  @ApiResponse({
    status: 400,
    description: 'Erro ao listar movimentações financeiras',
  })
  @ApiResponse({ status: 404, description: 'Loja não encontrada' })
  @ApiResponse({ status: 500, description: 'Erro interno do servidor' })
  ...
  @ApiResponse({
    status: 200,
    description: 'Movimentação financeira removida com sucesso',
  })
  @ApiResponse({
    status: 400,
    description: 'Erro ao remover movimentação financeira',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Movimentação financeira editada com sucesso',
    type: ResponseMovimentacaoFinanceiraDTO,
  })
  @ApiResponse({
    status: 400,
    description: 'Erro ao editar movimentação financeira',
  })
  @ApiResponse({ status: 404, description: 'Movimentação não encontrada' })
  @ApiResponse({ status: 500, description: 'Erro interno do servidor' })
  async editarMovimentacao(
    ...
    const lojaId = req.user.lojaId;
    return this.financeiroService.editarMovimentacao(
      id,
      lojaId,
      dto.metodoPagamento,
      dto.valor,
